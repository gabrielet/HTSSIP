...
filter_l2fc = function(df_l2fc, padj_cutoff=0.1){
  ...
  ### filtering
  df_l2fc = df_l2fc %>%
    dplyr::filter(rlang::eval_tidy(rlang::parse_expr("sparsity_threshold==BEST_SPAR_THRESH")))
  ...
  df_l2fc = df_l2fc %>%
    ...
    dplyr::filter(rlang::eval_tidy(rlang::parse_expr("log2FoldChange == max(log2FoldChange)"))) %>%
    dplyr::ungroup()
...
HRSIP = function(physeq,
  ...
  df_l2fc = df_l2fc %>%
    dplyr::group_by(sparsity_threshold) %>%
    dplyr::mutate(padj = rlang::eval_tidy(rlang::parse_expr("stats::p.adjust(p, method=padj_method)"))) %>%
    dplyr::ungroup()
